...
def main():
    ...
    for t in tokens:
        if t not in token_dict:
            ...
            Tokens.append(t)
    tokens = Tokens
    indices = np.array([[token_dict[token] for token in tokens] + [0] * (seq_len - len(tokens))])
    segments = np.array([[0] * len(tokens) + [0] * (seq_len - len(tokens))])
    masks = np.array([[0, 1, 1] + [0] * (seq_len - 3)])
    predicts = model.predict([indices, segments, masks])[0].argmax(axis=-1).tolist()
    print('Fill with: ', list(map(lambda x: token_dict_inv[x], predicts[0][1:-1])))
def test():
    import tensorflow as tf
